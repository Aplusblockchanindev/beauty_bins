...
class J2StoreModelProductsBehaviorVariableSubscriptionproduct extends F0FModelBehavior {
	...
	{
		if(!isset($data['product_type']) || $data['product_type'] != 'variablesubscriptionproduct') return;
		...
		if($data['j2store_product_id'] ){
			...
			if($product->params){
				...
				if(!isset($data['params']) || empty($data['params'])) {
					...
					$data['params']->subscriptionproduct->recurring_type = 'multiple';
					if(isset($data['params']->subscriptionproduct->subscription_period_units) && isset($data['params']->subscriptionproduct->subscription_length)){
						if($data['params']->subscriptionproduct->subscription_period_units == $data['params']->subscriptionproduct->subscription_length){
							$data['params']->subscriptionproduct->recurring_type = 'single';
						}
					}
			...
			if(isset($this->_rawData['variable'])){
				foreach($this->_rawData['variable'] as $key => $item){
					...
					if(isset($item->params)){
						$item->params->subscriptionproduct->recurring_type = 'multiple';
						if(isset($item->params->subscriptionproduct->subscription_period_units) && isset($item->params->subscriptionproduct->subscription_length)){
							if($item->params->subscriptionproduct->subscription_period_units == $item->params->subscriptionproduct->subscription_length){
								$item->params->subscriptionproduct->recurring_type = 'single';
							}
						}
	...
	public function onBeforeDelete(&$model) {
		...
		if($product->load($id)) {
			if($product->product_type != 'variablesubscriptionproduct') return;
			$variantModel = F0FModel::getTmpInstance('Variants', 'J2StoreModel');
