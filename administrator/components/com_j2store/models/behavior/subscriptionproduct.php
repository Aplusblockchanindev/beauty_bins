...
class J2StoreModelProductsBehaviorSubscriptionproduct extends F0FModelBehavior {

    private $_rawData = array();

    public function onAfterGetItem(&$model, &$record) {
        //sanity check
        if($record->product_type != 'subscriptionproduct') return;
        ...
        if($record->j2store_product_id) {
            ...
            try {
                ...
                //lets load product bundle as well
                $record->product_bundle = $record->params->get('subscriptionproduct',array());
    ...
    {
        if(!isset($data['product_type']) || $data['product_type'] != 'subscriptionproduct') return;
    ...
    public function onAfterSave(&$model) {
        if($this->_rawData) {
            ...
            if($table->product_type != 'subscriptionproduct') return;
    ...
    public function onBeforeDelete(&$model) {
        ...
        $product = F0FTable::getAnInstance('Product', 'J2StoreTable')->getClone();
        if($product->load($id)) {
            if($product->product_type != 'subscriptionproduct') return;
            $variantModel = F0FModel::getTmpInstance('Variants', 'J2StoreModel');
    ...
    public function onAfterGetProduct(&$model, &$product) {
        //sanity check
        if(!in_array($product->product_type, array('subscriptionproduct'))) return;
