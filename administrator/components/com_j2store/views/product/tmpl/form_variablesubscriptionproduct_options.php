...
<div class="j2store-product-variants">
	<div class="form-group">
		...
		<?php
		$hasOption = 0;
		if(isset($this->item->product_options ) && !empty($this->item->product_options)):
			$hasOption = 1;
			?>
			...
			<tr class="j2store_a_options">
				<td colspan="3">
					...
					<?php
					if($hasOption)
						echo J2Html::text('Selectoption' ,'',array('id'=>'optionselector', 'disabled' => 'disabled'));
					else
						echo J2Html::text('Selectoption' ,'',array('id'=>'optionselector'));
					?>
					<div class="alert alert-block alert-info">
						<?php echo JText::_('J2STORE_SUBSCRIPTION_VARIABLESUBSCRIPTIONPRODUCT_ALLOWED_ONLY_ONE_OPTION'); ?>
					</div>
				</td>
			</tr>
...
function removePAOption(pao_id) {
	(function($) {
		var variable_option_remove = {
			...
			product_type: 'variablesubscriptionproduct',
			pao_id: pao_id
		};
		$.ajax({
			type : 'post',
