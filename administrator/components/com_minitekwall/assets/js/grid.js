(function( $ ) {
	...
	function initDesigner()
	{
		...
		$grid.on('click', '.gridP-remove', function(event) {
		  $grid.packery('remove', event.currentTarget.closest('.gridP-item'))
		...
		$('.gridOptions').on('click', '#update-columns', function(event) {
			...
			$('.gridContainer').removeClass(function (index, className) {
			...
			$grid.find('.gridP-item').each(function(i, gridItem) {
				...
				while (item_width > grid_width)
				{
					...
					item_width = (new_data_width * columnWidth) + ((new_data_width - 1) * 5); /* n * columnWidth + gutters */
				}
				fixItemDimensions(gridItem, columnWidth, cellHeight);
			});
		...
		$grid.on('click', '.gridP-edit', function(event) {
		  ...
		  $('a[href="#items"]').tab('show');
			...
			document.getElementById('edit-height').value = parseInt($(editItem).attr('data-height'), 10);
			document.getElementById('edit-width').value = parseInt($(editItem).attr('data-width'), 10);
		});
		...
		$('#add-item').on('click', function() {
		  ...
		  var $items = $(''+
			'<div class="gridP-item '+ appended_class +'" data-width="'+ appended_data_width +'" data-height="'+ appended_data_height +'">'+
				'<span class="item-number"></span> '+
				'<span class="item-size">'+ appended_size +'</span>'+
		...
		function fixItemDimensions(gridItem, columnWidth, cellHeight) {
			...
			var itemWidth = (data_width * columnWidth) + ((data_width - 1) * 5); /* n * columnWidth + gutters */
			$(gridItem).css('width', itemWidth + 'px');

			// set items css height
			var data_height = parseInt($(gridItem).attr('data-height'), 10);
			var itemHeight = (data_height * cellHeight) + ((data_height - 1) * 5); /* n * cellHeight + gutters */
		...
		function orderItems() {
		  ...
		  $(itemElems).each(function(i, itemElem) {
		    $(itemElem).find('.item-number').text( i + 1 );
		...
		function createGridHtml(elements) {
			...
			$(elements).each(function(i, elem) {
				var elem_html = '';
				elem_html += '<div class="' + elem.class + '" data-width="' + elem.width + '" data-height="' + elem.height + '" data-index="' + elem.index + '">';
				...
				elem_html += '<span class="gridP-edit"><i class="fa fa-pencil"></i></span> ';
				elem_html += '<span class="gridP-remove"><i class="fa fa-times"></i></span>';
				elem_html += '</div>';
				elem_html += '</div>';
				html += elem_html;
			});
